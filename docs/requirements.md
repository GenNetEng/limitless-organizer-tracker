...
| FR28 | Admin config edit API + frontend: `PUT /api/admin/config` accepts partial updates validated against the editable-keys allowlist, persists via `set_config_value()`, and returns the updated effective config. Frontend `AdminConfig.tsx` supports inline editing with per-row edit/save controls via `useMutation` | BR1 | Pending — Phase 36 ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
...
| 36 | Admin config edit API + frontend: `PUT /api/admin/config` with allowlist validation, `AdminConfig.tsx` inline editing (FR28) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
