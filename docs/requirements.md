...
| FR24 | Log a `scraper.session_refreshed` event in the event log when `authenticated_page()` detects and recovers from an expired session, so the recovery is visible in the admin Event Log viewer (not just Celery worker logs) | BR1 | Pending — Phase 31 ([#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96)) |
...
| 31 | Log `scraper.session_refreshed` event on expired session recovery (FR24) ([#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96)) | MVP4 | [#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96) |
