...
| FR26 | Runtime configuration database: `config_entries` table stores key/value pairs that override env-var defaults at runtime. `config_db.py` module provides `get_config_value()`, `set_config_value()`, and an `EDITABLE_CONFIG_KEYS` allowlist restricting which settings are admin-editable. Sensitive config (DB URL, credentials, API keys) is never stored in the DB | BR1 | Pending — Phase 34 ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
...
| 34 | Config DB table + model + migration: `ConfigEntry` model, `config_db.py` module with `get_config_value()`, `set_config_value()`, `EDITABLE_CONFIG_KEYS` allowlist (FR26) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
