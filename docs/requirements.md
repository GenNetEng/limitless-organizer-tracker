...
| FR19 | Centralized event log: all significant application operations (task lifecycle, scraper actions, data ingestion, notifications, API triggers) are recorded in a partitioned `event_log` table with event type, severity, source, message, and JSON metadata. Provides an `app/events.py` service callable from any module | BR1 | **Phase 21a** (`app/events.py`, `app/db/models.py`) |
| FR20 | Admin event-log API: `GET /api/admin/event-log` returns paginated, filterable event log entries (by event type, severity, source, date range) | BR1 | **Phase 21a** (`app/api/routers/admin.py`) |
| FR21 | Admin diagnostics API: `GET /api/admin/diagnostics` returns system health (DB status, Redis connectivity, Celery worker/beat status, last successful run per task) | BR1 | **Phase 21a** (`app/api/routers/admin.py`) |
| FR22 | Admin config API: `GET /api/admin/config` returns current non-sensitive configuration (scan intervals, backfill months, ingest limits) via hardcoded allowlist | BR1 | **Phase 21a** (`app/api/routers/admin.py`) |
| FR23 | Admin tasks API: `GET /api/admin/tasks` returns available task trigger endpoints for frontend task-trigger buttons | BR1 | **Phase 21a** (`app/api/routers/admin.py`) |
...
| 21b | Admin frontend: Admin tab with EventLog, Diagnostics, TaskTriggers, Config components | MVP3 | [#70](https://github.com/GenNetEng/limitless-organizer-tracker/issues/70) |
