...
| FR24 | Log a `scraper.session_refreshed` event in the event log when `authenticated_page()` detects and recovers from an expired session, so the recovery is visible in the admin Event Log viewer (not just Celery worker logs) | BR1 | **Done — Phase 31** ([#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96)) |
| FR25 | When `SCRAPER_DEBUG` is enabled, capture full page HTML (`page.content()[:20000]`) on every scrape attempt (not just failures) and include it in the `log_event()` details, providing on-demand diagnostic visibility without a code change or redeploy | BR1 | **Done — Phase 33** ([#91](https://github.com/GenNetEng/limitless-organizer-tracker/issues/91)) |
| FR26 | Runtime configuration database: `config_entries` table stores key/value pairs that override env-var defaults at runtime. `config_db.py` module provides `get_config_value()`, `set_config_value()`, and an `EDITABLE_CONFIG_KEYS` allowlist restricting which settings are admin-editable. Sensitive config (DB URL, credentials, API keys) is never stored in the DB | BR1 | **Done — Phase 34** ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
| FR27 | Config resolution layer: `get_effective_config()` merges DB overrides over env-var defaults for all editable keys; `get_effective_value()` resolves a single key with type coercion. `GET /api/admin/config` returns effective (DB-merged) config. Runtime task config reads (`tournament_ingest_limit`, `tournament_backfill_months`, `organizer_scan_start_id`, `organizer_scan_limit`) use `get_effective_value()` instead of raw `settings.*` | BR1 | **Done — Phase 35** ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
| FR28 | Admin config edit API + frontend: `PUT /api/admin/config` accepts partial updates validated against the editable-keys allowlist, persists via `set_config_value()`, and returns the updated effective config. Frontend `AdminConfig.tsx` supports inline editing with per-row edit/save controls via `useMutation` | BR1 | **Done — Phase 36** ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
| FR29 | Dynamic Celery beat schedule via `celery-redbeat`: replaces the static import-time beat schedule with Redis-backed `RedBeatSchedulerEntry` objects. `build_beat_schedule()` reads effective config (DB-merged) and writes schedule entries to Redis. Schedule is rebuilt on app startup and after `PUT /api/admin/config` writes, so config changes take effect without redeploying | BR1 | **Done — Phase 37** ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) |
| FR30 | Surface `detected_at` with full datetime in organizer API responses; `GET /api/organizers/recently-onboarded?limit=N` returns the N most recently detected organizers ordered by `detected_at` desc; `display_timezone` admin-editable setting (default `America/Chicago`) | BR3 | **Done — Phase 40** ([#101](https://github.com/GenNetEng/limitless-organizer-tracker/issues/101), [#102](https://github.com/GenNetEng/limitless-organizer-tracker/issues/102), [#126](https://github.com/GenNetEng/limitless-organizer-tracker/issues/126)) |
...
| 24 | Performance: fix N+1 queries in tournament ingestion ([#54](https://github.com/GenNetEng/limitless-organizer-tracker/issues/54)), add index on `OrganizerActivity.game` ([#31](https://github.com/GenNetEng/limitless-organizer-tracker/issues/31)) — **Done** | — | [#54](https://github.com/GenNetEng/limitless-organizer-tracker/issues/54), [#31](https://github.com/GenNetEng/limitless-organizer-tracker/issues/31) |
| 25 | Scrape endpoint syncs `first_tournament_date` + returns `estimated_onboard_date` from regression ([#62](https://github.com/GenNetEng/limitless-organizer-tracker/issues/62)) — **Done** | MVP3 | [#62](https://github.com/GenNetEng/limitless-organizer-tracker/issues/62) |
| 26 | Historical tournament backfill: full-history ingestion task with admin trigger ([#68](https://github.com/GenNetEng/limitless-organizer-tracker/issues/68)) — **Done** | MVP3 | [#68](https://github.com/GenNetEng/limitless-organizer-tracker/issues/68) |
| 27 | Error-state tests for admin + data components ([#75](https://github.com/GenNetEng/limitless-organizer-tracker/issues/75)) | MVP3 | [#75](https://github.com/GenNetEng/limitless-organizer-tracker/issues/75) |
| 28 | Wait-estimate response shape cleanup: remove `intercept`, add `fitted_line` endpoints + `total_points`, cap `points` at 200 (FR12, FR13) ([#37](https://github.com/GenNetEng/limitless-organizer-tracker/issues/37)) | MVP3 | [#37](https://github.com/GenNetEng/limitless-organizer-tracker/issues/37) |
| 29 | Expired session detection and recovery: `authenticated_page()` validates stored sessions, auto-refreshes on expiry ([#16](https://github.com/GenNetEng/limitless-organizer-tracker/issues/16)) | MVP3 | [#16](https://github.com/GenNetEng/limitless-organizer-tracker/issues/16) |
| 30 | Triage resubmit bug: verified fix from PR #88, added missing `post_request_failed` test (FR3) ([#86](https://github.com/GenNetEng/limitless-organizer-tracker/issues/86)) | MVP4 | [#86](https://github.com/GenNetEng/limitless-organizer-tracker/issues/86) |
| 31 | Log `scraper.session_refreshed` event on expired session recovery (FR24) ([#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96)) — **Done** | MVP4 | [#96](https://github.com/GenNetEng/limitless-organizer-tracker/issues/96) |
| 32 | Configurable session validation timeout: `SESSION_VALIDATION_TIMEOUT_MS` setting controls `page.goto()` timeout during session validation; `PlaywrightTimeoutError` treated as expired session ([#97](https://github.com/GenNetEng/limitless-organizer-tracker/issues/97)) — **Done** | MVP4 | [#97](https://github.com/GenNetEng/limitless-organizer-tracker/issues/97) |
| 33 | `SCRAPER_DEBUG` config flag: when enabled, capture full page HTML on every scrape attempt in event log details (FR25) ([#91](https://github.com/GenNetEng/limitless-organizer-tracker/issues/91)) | MVP4 | [#91](https://github.com/GenNetEng/limitless-organizer-tracker/issues/91) |
| 34 | Config DB table + model + migration: `ConfigEntry` model, `config_db.py` module with `get_config_value()`, `set_config_value()`, `EDITABLE_CONFIG_KEYS` allowlist (FR26) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) — **Done** | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
| 35 | Config resolution: DB overrides env (FR27) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) — **Done** | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
| 36 | Admin config edit API + frontend: `PUT /api/admin/config` with allowlist validation, `AdminConfig.tsx` inline editing (FR28) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) — **Done** | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
| 37 | Dynamic Celery beat schedule via `celery-redbeat`: Redis-backed beat entries rebuilt on startup and after config edits (FR29) ([#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100)) — **Done** | MVP4 | [#100](https://github.com/GenNetEng/limitless-organizer-tracker/issues/100) |
| 38 | UI styling rework: DaisyUI dim theme, table zebra, pagination on all lists, task trigger table with component badges, error message standardization, auto-refresh, keepPreviousData ([#79](https://github.com/GenNetEng/limitless-organizer-tracker/issues/79)) — **Done** | MVP4 | [#79](https://github.com/GenNetEng/limitless-organizer-tracker/issues/79) |
| 39 | UI layout + table polish: widen page layout (#122), fix zebra striping contrast in dim theme (#123), reorganize dashboard tabs (#124), fix MSW PUT handler type mismatch (#116) — **Done** | MVP4 | [#122](https://github.com/GenNetEng/limitless-organizer-tracker/issues/122), [#123](https://github.com/GenNetEng/limitless-organizer-tracker/issues/123), [#124](https://github.com/GenNetEng/limitless-organizer-tracker/issues/124), [#116](https://github.com/GenNetEng/limitless-organizer-tracker/issues/116) |
...
| 47 | Backfill Organizer rows from tournament data: one-time admin-triggered Celery task that creates Organizer rows for all orphan `organizer_id`s in tournaments table, with trigger endpoint and admin UI entry (FR17) — **Done** | MVP5 | [#136](https://github.com/GenNetEng/limitless-organizer-tracker/issues/136) |
| 48 | Historical organizer ID scan 1–watermark: probes organizer IDs below the scanner watermark for existence, does not stop at first 404 (FR17) — **Done** | MVP5 | [#137](https://github.com/GenNetEng/limitless-organizer-tracker/issues/137) |
| 49 | Verify frontier regression + refactor: extracted `build_frontier_regression()` into `app/analytics/frontier.py`, added a verification task logging slope/R²/frontier_size/sample_size after backfill (FR12) — **Done** | MVP5 | [#139](https://github.com/GenNetEng/limitless-organizer-tracker/issues/139) |
| 50 | Documentation site: scaffold + project/dev/deployment docs: MkDocs + mkdocs-material, full nav, `docs-build` CI job — **Done** | MVP5 | [#140](https://github.com/GenNetEng/limitless-organizer-tracker/issues/140) |
| 51 | Documentation site: metrics docs (frontier regression methodology, organizer activity, organizer lifecycle) | MVP5 | [#140](https://github.com/GenNetEng/limitless-organizer-tracker/issues/140) |
| 52 | Deploy docs site via Helm + Ingress: dedicated `docs` image/Deployment/Service, served at `/manual` via a new Ingress resource | MVP5 | [#147](https://github.com/GenNetEng/limitless-organizer-tracker/issues/147) |
| 53 | MVP5 verification + release cut (v0.5.0): update requirements traceability, record remaining technical decisions, CHANGELOG cut, full test suite, manual staging verification | MVP5 | — |
