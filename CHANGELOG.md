...
### Tests
- Unit tests for the `ApplicationStatus` enum and model table names.
- Integration tests for `/healthz` and for creating/querying
  `Tournament` and `ApplicationStatusCheck` rows against an in-memory
  SQLite database.
