...
def test_ingest_creates_organizer_first_tournament_date(session):
    ...
    organizer = session.get(Organizer, 100)
    assert organizer is not None
    assert organizer.first_tournament_date == date(2026, 5, 1)


def test_ingest_updates_existing_organizer_first_tournament_date(session):
    ...
    assert session.get(Organizer, 100).first_tournament_date == date(2026, 6, 5)

    ingest_tournaments(session, [_dto("t0", 100, "PTCG", datetime(2026, 5, 1, tzinfo=timezone.utc))])

    assert session.get(Organizer, 100).first_tournament_date == date(2026, 5, 1)


def test_ingest_does_not_overwrite_onboarded_at(session):
...
def test_ingest_creates_organizer_rows_for_multiple_organizers(session):
    dtos = [
    ...
    assert session.get(Organizer, 100).first_tournament_date == date(2026, 6, 1)
    assert session.get(Organizer, 200).first_tournament_date == date(2026, 6, 5)
