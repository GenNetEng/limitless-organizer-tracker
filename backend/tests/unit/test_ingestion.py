...
def test_ingest_creates_tournament_rows(session):
    dto = _dto("t1", 100, "PTCG", datetime(2026, 6, 1, tzinfo=timezone.utc))

    ingest_tournaments(session, [dto])

    tournament = session.get(Tournament, "t1")
    assert tournament is not None
    assert tournament.organizer_id == 100
    assert tournament.game == "PTCG"


def test_ingest_sets_first_and_last_seen_for_organizer(session):
    dtos = [
    ...
    activity = session.get(OrganizerActivity, (100, "PTCG"))
...
def test_ingest_tracks_separate_activity_per_game(session):
    dtos = [
        _dto("t1", 100, "PTCG", datetime(2026, 6, 5, tzinfo=timezone.utc)),
    ...
    ptcg = session.get(OrganizerActivity, (100, "PTCG"))
    vgc = session.get(OrganizerActivity, (100, "VGC"))
...
def test_reingesting_earlier_tournament_updates_first_seen(session):
    ingest_tournaments(session, [_dto("t1", 100, "PTCG", datetime(2026, 6, 5, tzinfo=timezone.utc))])
    activity = session.get(OrganizerActivity, (100, "PTCG"))
    assert activity.first_tournament_date == datetime(2026, 6, 5, tzinfo=timezone.utc)

    ingest_tournaments(session, [_dto("t0", 100, "PTCG", datetime(2026, 5, 1, tzinfo=timezone.utc))])
...
def test_reingesting_same_tournament_updates_in_place(session):
    ingest_tournaments(session, [_dto("t1", 100, "PTCG", datetime(2026, 6, 5, tzinfo=timezone.utc), players=10)])
    ingest_tournaments(session, [_dto("t1", 100, "PTCG", datetime(2026, 6, 5, tzinfo=timezone.utc), players=42)])

    count = session.query(Tournament).count()
    assert count == 1
    assert session.get(Tournament, "t1").players == 42
